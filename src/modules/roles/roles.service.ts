...
export class RolesService extends AbstractService {
  ...
  async create(createRolesDto: CreateUpdateRoleDto, permissionsIds: { id: string }[]): Promise<Role> {
    try {
      const role = this.rolesRepository.create({ ...createRolesDto, permissions: permissionsIds })
